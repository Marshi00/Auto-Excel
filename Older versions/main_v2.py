...
def replace_placeholders(template_file, result_row, output_file, placeholders):
    try:
        ...
        for index, row in template_df.iterrows():
            # Replace placeholders with device type, device name, and location
            updated_row = row.apply(lambda cell: cell.replace(placeholders['type'], result_row['Device_Type']))
            updated_row = updated_row.apply(lambda cell: cell.replace(placeholders['loc'], result_row['location']))
            updated_row = updated_row.apply(lambda cell: cell.replace(placeholders['name'], result_row['Device_Name']))
...
def main():
    ...
    for result_index, result_row in result_df.iterrows():
        ...
        replace_placeholders(template_file, result_row, output_file, placeholders)
        print(f"Processed {result_index}/{len(result_df) - 1}")

        if template_file not in os.listdir('../templates'):
